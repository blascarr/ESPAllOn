...
Control *typeGPIOController;	// Control Type GPIO ( Input or OUTPUT )
...
template <ESPinner_Mod ModType, const char *ConfigKey>
class ESPinner_SimpleGPIO : public ESPinner {
  protected:
	uint8_t gpio;

  public:
	ESPinner_SimpleGPIO() : ESPinner(ModType) {}

	void setGPIO(uint8_t gpio_pin) { gpio = gpio_pin; }
	uint8_t getGPIO() const { return gpio; }

	JsonDocument serializeJSON() override {
		StaticJsonDocument<256> doc;
		doc[ESPINNER_MODEL_JSONCONFIG] = ConfigKey;
		doc[ESPINNER_ID_JSONCONFIG] = getID();
		doc[ESPINNER_GPIO_JSONCONFIG] = getGPIO();
		return doc;
	}

	bool deserializeJSON(const String &data) override {
		StaticJsonDocument<256> doc;
		DeserializationError error = deserializeJson(doc, data);
		if (error)
			return false;

		setID(doc[ESPINNER_ID_JSONCONFIG].as<const char *>());
		setGPIO(doc[ESPINNER_GPIO_JSONCONFIG].as<int>());
		return true;
	}
};

class ESPinnerControlFactory {
  public:
	template <typename ModType>
	static void createBasicSelector(uint16_t parentRef, ModType *mod) {
		// Lógica común para crear selectores GPIO
	}

	template <typename ModType>
	static void createSaveRemoveButtons(
		uint16_t parentRef, std::function<void(Control *, int)> saveCallback,
		std::function<void(Control *, int)> removeCallback) {
		// Lógica común para botones Save/Remove
	}
};

template <typename ModType> class ESPinnerCallbackManager {
  public:
	static void genericAction(uint16_t parentRef);
	static void genericSaveCallback(Control *sender, int type);
	static void genericRemoveCallback(Control *sender, int type);
	static void genericSelectorCallback(Control *sender, int type);
};