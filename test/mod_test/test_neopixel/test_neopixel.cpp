 ...
 *
 * This basic test validates the ESPinner_NeoPixel module initialization and UI
 * creation.
 *
 * Test Steps:
 ...
 * 4. Create NeoPixel ESPinner through UI selection
 * 5. Validate NeoPixel-specific UI elements are properly created
 */
...
void setup() {
	...
	uint16_t neopixel_ref =
		searchByLabel(elementToParentMap.begin()->second, "ESPinnerType");
	Control *typeNEOPIXELController = ESPUI.getControl(neopixel_ref);
	typeNEOPIXELController->value = NEOPIXEL_LABEL;
	saveElement_callback(typeNEOPIXELController, B_UP);
